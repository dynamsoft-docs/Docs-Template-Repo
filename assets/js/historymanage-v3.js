...
function addParam(aTag, verText, fromSourse = null, needh3 = false) {
  ...
  if (
    ...
    getCurrentUrlLang(hrefVal, true) == "objectivec-swift"
  ) {
  ...
  if (
    ...
    !getUrlVars(hrefVal)["src"]
  ) {
    if (getUrlVars(document.URL)["src"]) {
      srcString =
        hrefVal.indexOf("?") < 0
          ? "?src=" + getUrlVars(document.URL)["src"]
          : "&src=" + getUrlVars(document.URL)["src"];
    } else {
      srcString =
        hrefVal.indexOf("?") < 0
          ? "?src=" + getCurrentUrlLang(document.URL)
          : "&src=" + getCurrentUrlLang(document.URL);
    }
  }
  ...
  if (aTag.target == "_blank") {
    if (getUrlVars(originHref)["ver"] != undefined) {
      ...
      currentVersion =
        currentVersion.indexOf("latest version") >= 0
          ? "latest"
...
async function changeVersion(liTag) {
  ...
  let srcVal = getUrlVars(curUrl)["src"];
  let langVar = getUrlVars(curUrl)["lang"];
  let productVar = getUrlVars(curUrl)["product"];
  let anchorVar = undefined;
  if (curUrl.indexOf("#") != -1) {
...
async function initHistoryVersionList() {
  ...
  return new Promise((resolve, reject) => {
    fetch(treePageLink, { cache: "no-cache" })
      .then(function (response) {
        return response.text();
      })
      .then(function (data) {
    ...
    .then(function (data) {
      ...
      if ($(historyVersion) && $(historyVersion).length > 0) {
        $("#categoryMenuTree_history .fullVersionHistory").html(
          $(historyVersion).html()
        );
      }
...
function initHistoryVersionList_Detail() {
  ...
  $(".currentVersion").text("Latest Version (" + latestVersion + ")");
}
// #endregion
...
function getRequestNewPageVersion(linkUrl) {
  ...
  if (queryProduct == undefined) {
    // same product
    if (queryLang == undefined || linkUrl.indexOf("/core/") < 0) {
      // same product, same language
      return curVersion;
    } else {
      // same product, lang to core
      if (curVersion == "latest") {
        var searchLang = queryLang;
        var productName = getCurrentUrlProductName(linkUrl);
        // var langLV = getProductLangLatestVersion(
        //   DcvProducts.indexOf(productName) >= 0 ? "dcv" : productName,
        //   searchLang
        // );
        // var coreLV = getProductLangLatestVersion(
...
function getLinkVersion(
  ...
  lang = ["objectivec-swift", "objectivec", "objc", "swift"].includes(lang)
    ? "ios"
    : lang;
  lang = lang == "core" ? "" : lang;
