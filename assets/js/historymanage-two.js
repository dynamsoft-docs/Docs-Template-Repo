var redirectList = null
getRedirctVersionList();
function UrlReplace()
{
...
function RequestNewPage(aTag, paramLink, onlyLoadContent=false) {
    ...
    fetch(aTag.href).then(function(response) {
        ...
        // dwt redirect links
        var dwtRedirectList = $(data).find("#dwtRedirectList").html()
        $("#dwtRedirectList").html(dwtRedirectList)
...
function findCurLinkOnFullTree(aTag, paramLink, onlyLoadContent=false, isRequestNewPage = false) {
    ...
    if (curDocUrl == targetHref && (aTag.href.split("#").length > 1 || document.URL.split("#").length > 1) && !isRequestNewPage) {
        ...
        if (!flag) {
            if (document.URL.toLowerCase() != paramLink.toLowerCase()) {
...
async function changeVersion(liTag) {
    ...
    if (curUrl.indexOf("web-twain/docs-archive/v19.0") > 0 && GetVersionDiff('17.2.1', ver) < 0) {
        verFileName = '/v17.2.1/';
    }

    let itemRedirectList = redirectList && redirectList.filter((item) => {
        return curUrl.toLowerCase().indexOf(item.url.toLowerCase()) >= 0 && GetVersionDiff(item.version, ver) <= 0
    })

    if (curUrl.indexOf("web-twain/docs/") > 0 && itemRedirectList && itemRedirectList.length > 0) {
        curUrl = location.origin + "/web-twain/docs" + itemRedirectList[0].redirect_from
    }

    if (GetVersionDiff('18.4', ver) <= 0) {
        if ($("#dwtRedirectList .dwtReditectLink").length > 0) {
            var redirectUrl = $("#dwtRedirectList .dwtReditectLink").val()
            curUrl = redirectUrl
        }
    }
    ...
    if (curUrl.indexOf("web-twain/docs-archive/v19.0/") > 0 && ver == "latest") {
        curUrl = curUrl.replace("/docs-archive/v19.0/", "/docs/")
        window.location.href = curUrl
        return
    } else {
        curUrl = curUrl.replace("/docs-archive/v19.0/", "/docs-archive" + verFileName)
        window.location.href = curUrl;
        return;
    }
}
...
function getRedirctVersionList(needArchive = false) {
    try {
        fetch(`${location.origin}/webres/wwwroot/js/docs/dwtRedirect.json`, {
            cache: "no-cache"
        }).then(response => {
            return response.text()
        }).then(function (data) {
            redirectList = JSON.parse(data)
        }).catch(error => {
        })
    } catch (error) {
    }
